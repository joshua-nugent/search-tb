...
############## EDITED for SEARCH-TB by Josh Nugent
...
Stage2 <- function(goal='aRR', weighting='clust', data.input,
                   ...
                   do.data.adapt = F, break.match = F, verbose = F){	

  ###### TRANSFORM data.input here........................ scale by max value
	...
	# Output 'est' has been unscaled
	est <- do.TMLE(goal = goal, weighting = weighting, train = data.input,  QAdj = QAdj,  
			 gAdj = gAdj, verbose = verbose, scale_value = scale_value, scale_value_min = scale_value_min)
	...
	if(break.match){
		...
		df <- n.pair - 1 
		inference <- get.inference(goal=goal, R1=R1, R0=R0, var.IC= est$var.pair, df=df)		
	}
...
do.TMLE <- function(goal, train, weighting='clust', 
	...
	# unscaling happens inside get.IC.variance()
	variance.out <- get.IC.variance(goal=goal, Vdata=train, R1=R1, R0=R0, scale_value = scale_value, scale_value_min = scale_value_min)

	RETURN <- list(train=train, 	
		QAdj=Q$QAdj, Q.out=Q$glm.out,
		gAdj=G$gAdj, p.out=G$p.out, 
		eps=eps, 
		R1 = R1*(scale_value - scale_value_min) + scale_value_min, ########################### Unscale
		R0 = R0*(scale_value - scale_value_min) + scale_value_min, ########################### Unscale
...
do.Init.Qbar<- function(train, QAdj, glm.out=NULL, verbose=F){
	...
	if( is.null(glm.out) ){
		...
		glm.out<- suppressWarnings( glm( Y~. , family='binomial', data=train.temp, weights=train$alpha ) )	#binomial
...
get.IC.variance <- function(goal, Vdata, R1, R0,
  ...
  #################### UNSCALE HERE, for DY1, DY0, R1, R0 ....
  ...
  #print("Vdata")
  #print(Vdata)
	#print("Vdata$alpha*Vdata$H.1W*(Vdata$Y - Vdata$Qbar1W.star)")
	#print(Vdata$alpha*Vdata$H.1W*(Vdata$Y - Vdata$Qbar1W.star))
	#print("scale_value - scale_value_min")
	#print(scale_value - scale_value_min)

	DY1 <- Vdata$alpha*Vdata$H.1W*(Vdata$Y - Vdata$Qbar1W.star)*(scale_value - scale_value_min) + scale_value_min
	DY0 <- Vdata$alpha*Vdata$H.0W*(Vdata$Y - Vdata$Qbar0W.star)*(scale_value - scale_value_min) + scale_value_min
	# print("DY1")
	# print(DY1)
	# print("DY0")
	# print(DY0)
	# aggregate DY1 to the cluster-level using household level ... for _14 versions to both arms
	# for _42: use JS household code to get 
	...
	#print(list(var.R1=var.R1, var.R0=var.R0, DY=DY, var.break=var.break, DY.paired=DY.paired, var.pair=var.pair))
...
get.inference <- function(goal, R1, R0, var.IC, df, sig.level = 0.05,
                          one_sided = T, alternate_null_negative = T){
  ...
  # cutoff based on t-dist for testing and CI	
  if(one_sided){
    #cutoff <- qt(sig.level, df = df, lower.tail=F)
